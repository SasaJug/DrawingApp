...
class ForgotPasswordViewModel @Inject constructor(
    ...
    private val newPasswordUseCase: NewPassword
) : BaseViewModel() {
    ...
    fun changePassword(username: String) {
        addDisposable(changePasswordUseCase.changePassword(username)
            .subscribe(
                { b: Boolean ->
                    if (b) {
                        val newPasswordChangeViewState = forgotPasswordLiveData.value?.copy(
    ...
    fun newPassword(code: String, password: String, confirmPassword: String) {
        ...
        if (errorCode > 0) {
            errorState.value =
                UIException("All entries must be valid", IllegalArgumentException(), errorCode)
            return
        }

        val passwordChangeViewState = forgotPasswordLiveData.value?.copy(
            passwordChangeStarted = true,
            loading = true,
            isPasswordChangeRequested = true,
            isPasswordChanged = false
